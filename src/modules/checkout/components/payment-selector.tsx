...
export default function PaymentSelector({
    ...
    if (method === "cod") {
      ...
      try {
        const res = await fetch("/api/payment/init", {
          ...
          body: JSON.stringify({ cartId: cart.id, providerId: codProviderId }),
        })
        if (!res.ok) {
      ...
      // For QR: init the session immediately too, reference saved separately
      setInitStatus("loading")
      try {
        const res = await fetch("/api/payment/init", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ cartId: cart.id, providerId: qrProviderId }),
        })
        if (!res.ok) {
          const body = await res.json().catch(() => ({}))
          throw new Error(body?.error ?? `Payment init failed (${res.status})`)
  ...
  async function saveReference() {
    if (!transactionId.trim()) {
      setErrorMsg("Please enter your Bank Reference / Transaction ID.")
      return
    }
    setRefStatus("saving")
    setErrorMsg("")
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {/* QR reference section — only shown after session is initialized */}
          ...
          <div className="p-4 flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-1">
              ...
              <input
                ...
                disabled={refStatus === "saved" || isReviewStep}
            ...
            <div className="flex flex-col gap-1">
              ...
              <div className="flex gap-2">
                <input
                  id="transaction-id"
                  type="text"
                  placeholder="e.g. BANK_REF_987654321"
                  value={transactionId}
                  onChange={(e) => {
                ...
                {!isReviewStep && (
                  <button
                    ...
                    disabled={refStatus === "saving" || refStatus === "saved"}
            ...
            {refStatus === "saved" && (
              <p className="text-xs text-emerald-600 flex items-center gap-1">
                ...
                Reference saved! You can now continue to review.
              </p>
            )}
          </div>
        </div>
      ...
      {!isReviewStep && (
        <div className="flex flex-col gap-2 pt-1">
          ...
          {selected === "qr" &&
            ...
            refStatus !== "saved" && (
              <p className="text-center text-xs text-gray-400">
                Confirm your transaction reference above to continue.
              </p>
            )}
        </div>
      )}
