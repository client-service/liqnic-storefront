...
import PaymentSelector from "@modules/checkout/components/payment-selector"
...
async function PaymentMethodSection({ cart }: { cart: HttpTypes.StoreCart }) {
  const paymentMethods = await listCartPaymentMethods(cart.region?.id ?? "")
  const codProvider = paymentMethods?.find((pm) =>
    pm.id.includes("cod-payment")
  )
  const qrProvider = paymentMethods?.find((pm) => pm.id.includes("qr-payment"))

  // Need at least COD to show the selector
  if (!codProvider) return null

  return (
    <PaymentSelector
      cart={cart}
      codProviderId={codProvider.id}
      qrProviderId={qrProvider?.id ?? ""}
    />
  )
}
...
export default async function CheckoutForm({
  ...
  return (
    <div className="w-full grid grid-cols-1 gap-y-8">
      ...
      <Suspense fallback={<SectionSkeleton />}>
        <PaymentMethodSection cart={cart} />
      </Suspense>
